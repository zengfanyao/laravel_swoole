...
class Server
{
    ...
    public function __construct()
    {
        ...
        $this->serv = new swoole_websocket_server("0.0.0.0", 9611);
        ...
        $port1 = $this->serv->listen("0.0.0.0", 9610, SWOOLE_SOCK_TCP);
    ...
    public function onMessage(swoole_websocket_server $_server, $frame)
    {
        ...
        switch ($cmd) {
            ...
            case "msg":
                ...
                if ($recv['socket_ip'] != "127.0.0.1:9610") {//发消息给proxy

                    // 转发服务器信息
                    $proxy = json_decode($this->redis->get('127.0.0.1:9610_router'));
                    //需要转发
